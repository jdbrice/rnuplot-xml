...
class Gnuplot
{
	...
	template<typename X>
	std::string df_x(const X& x, const std::string &title = ""){
		if (x.size() == 0)
		{
	...
	std::string df_xy(const X& x, const Y& y, const std::string &title = ""){
	...
	template <typename T>
	std::string df( std::string fn, const T& x, const T& y = T(), const T& z = T(), const T& w = T(), const T& v = T(), const T& a = T(), const T& b = T() , const T& c = T()){
		return df( x, y, z, w, v, a, b, c, fn );
	}

	template <typename T>
	std::string df( const T& x, const T& y = T(), const T& z = T(), const T& w = T(), const T& v = T(), const T& a = T(), const T& b = T(), const T& c = T(), std::string fn = ""){
		if (x.size() == 0 || y.size() == 0)
		{
			throw GnuplotException("std::vectors too small");
			return "";
